...
exports.getAllReturnInv = async (req, res) => {
  try {
    const queryObj = { ...req.query };
    ...
    let query = ReturnInvoice.find(queryObj);
    ...
    const numInvoice = await ReturnInvoice.countDocuments();
    const totalPages = Math.ceil(numInvoice / limit);
    ...
    res.status(200).json({
      message: `We've catched ${returnInvoices.length} returnInvoices successfully ✅`,
      pagination: {
        numInvoice,
        totalPages,
        currentPage: page,
...
exports.getReturnInvById = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: `We've catched the return inv num ${returnInv.invoiceNumber} successfully ✔`,
      data: returnInv,
    });
  } catch (error) {
    res.status(500).json({
...
exports.updateReturnInv = async (req, res) => {
  ...
  const {items, discount, customer} = req.body;
